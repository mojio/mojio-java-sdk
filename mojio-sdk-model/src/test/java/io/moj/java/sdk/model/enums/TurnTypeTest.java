...
public class TurnTypeTest extends EnumTest<TurnType> {

    @Override
    public Map<String, TurnType> getMapping() {
        // these are defined by the server's contract so should be safe to validate against in tests
        return new ImmutableMap.Builder<String, TurnType>()
                .put("Left", TurnType.LEFT)
                .put("Right", TurnType.RIGHT)
                .build();
    }
